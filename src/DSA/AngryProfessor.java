 ...
 * on time (arrivalTime < 0) to arrived late (
 * arrivalTime > 0).
 ...
 * or on time; positive arrival times (a[i] > 0) indicate the student arrived
 * a[i] minutes late.
 * Function Description
 * Complete the angry Professor function in the editor below. It must return YES
 * if class is cancelled, or NO otherwise. angry Professor has the following
 * parameter(s):
 ...
 * The first line has two space-separated integers, n and k, the number of
 * students (size of a) and the cancellation threshold.
...
/*
 * To solve the problem of determining whether the class is canceled based on
 * student arrival times and a threshold number of students, we need to count
 * how many students arrived on time (arrival time <= 0). If this count is less
 * than the threshold
 * 𝑘
 ...
 * 
 * We initialize a counter onTimeCount to count the number of students who
 * arrived on time.
 * We iterate through the list a of arrival times. If an arrival time is less
 * than or equal to 0, we increment the onTimeCount.
 * After counting, we compare onTimeCount with the threshold k. If onTimeCount
 * is less than k, we return "YES" (class is canceled); otherwise, we return
 * "NO" (class is not canceled).
 * main Method:
 * 
 * We use BufferedReader to read the input efficiently.
 * We read the number of test cases t.
 * For each test case, we read the values of n (number of students) and k
 * (threshold number of students).
 * We read the arrival times of the students into a list a.
 * We call the angryProfessor method with k and a as arguments and print the
 * result.
 * This approach ensures that we efficiently determine whether the class is
 * canceled based on the given conditions.
 */